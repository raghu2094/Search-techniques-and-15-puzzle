    ...
    for i in range(4):
     for j in range(4):
         if(G[i][j]==0):
             ...
             move_vertical(G,i,j)
    #print("length of successors is",len(s))
    #print("Successors are",s)
    return s
...
#To retrive value of the heuristic from the fringe for a given state
def get_heuristic(A,fringe):
...
def solve(matrix):
    ...
    initial_move='X00'
    fringe=[]
    heapq.heappush(fringe,(0, [matrix,initial_move]))
    temp=[]
    ...
    while len(fringe)>0:
        ...
        for s1 in successors(temp1):
            ...
            if len(fringe)>0:
                index,h1= get_heuristic(s1[0],fringe)
                if h1>h:
                    del fringe[index]
